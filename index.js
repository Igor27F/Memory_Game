...
function drawImage(number) {
  return sortedTheme == 0
    ? rickAndMortyApi + number + ".jpeg"
    : pokemonAPI + number + ".png";
}

//posicionar elementos na tela
function draw() {
  document.getElementById("gameInstructions").style.width =
    gameScreen.offsetWidth + "px";
  document.getElementById("gameInstructions").style.height =
    gameScreen.offsetHeight * 0.1 + "px";
  document.getElementById("timer").style.height =
    gameScreen.offsetHeight * 0.1 + "px";
  document.getElementById("instructionText").style.marginTop =
    document.getElementById("gameInstructions").offsetHeight * 0.5 + "px";
  if (memes[0]) {
    for (let i = 0; i < total; i++) {
      memes[i].style.height =
        (gameScreen.offsetHeight * (100 / numberOfRows)) / 100 + "px";
      memes[i].style.width =
        (gameScreen.offsetWidth * (100 / numberOfColumns)) / 100 + "px";
      memes[i].style.marginLeft =
        ((gameScreen.offsetWidth * (100 / numberOfColumns)) / 100) *
          (i + 1 > numberOfColumns ? i % numberOfColumns : i) +
        "px";
      memes[i].style.marginTop =
        (gameScreen.offsetHeight * 10) / 100 +
        ((gameScreen.offsetHeight * (100 / numberOfRows)) / 100) *
          parseInt(i / numberOfColumns) +
        "px";
    }
  }
}
...
function memorizeTime(memTime, playTime) {
  document.getElementById("timer").style.animation =
    "timer-animation-reverse " + memTime + "s linear";
...
Meme = function (positionX, positionY, imgNumber) {
  ...
  this.img.onclick = () => {
    if (isGameRunning) imageSelected(this.img, imgNumber);
  };
  ...
  this.img.style.marginLeft =
    ((gameScreen.offsetWidth * 25) / 100) * positionX + "px";
  this.img.style.marginTop =
    (gameScreen.offsetHeight * 10) / 100 +
    ((gameScreen.offsetHeight * 30) / 100) * positionY +
    "px";
};
  ...
  for (let i = 0; i < numberOfRows; i++) {
    for (let j = 0; j < numberOfColumns; j++) {
      randomNumber = Math.floor(
        Math.random() * (total - j - i * numberOfColumns)
      );
